...
app.partial.tumblr = function () {
	_(tumblr_api_read.posts).each(function (d, i) {
		...
		.sm(12).md(12).lg(12).xs(12).fontsize(13).css('background-image', 'url(' + d['photo-url-500'] + ')').css('background-size', 'cover').css('background-position', 'center');
		...
		single.on('click', function () {
			...
			$(single).lightGallery({
				...
				$('.lg-sub-html a').each(function () {
					$(this).html($(this).text()).attr('target', '_blank');
				});
			});
		});
	});
